    ...
    try:
        ...
        def check_validity(self):
            try:
                ...
                # while number < len(self.person_json_value["Person"]):
                # Creating the user for buying or selling a shares
                #   if self.person_json_value["Person"][number]["Name"] == name.title():
                # Verifying the user
                index = number
        ...
        def buy_share(self, index, name):
            ...
            if self.person_json_value['Person'][index]["Total Balance"] > amount_pay:
                ...
                with open("stock.json", "w") as jf:
                    # Changing the updated stock in a file
